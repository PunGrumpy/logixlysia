...
Send logs to [PostHog](https://posthog.com) as captured events. Each log becomes an event with dot-notation properties you can use in filters, insights, and cohorts. Logs carrying a user ID link to PostHog persons automatically.
...
- **High event volume** — exclude log levels you don't need with [`logFilter`](/docs/features/filtering) before they reach the transport.
