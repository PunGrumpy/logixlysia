...
Logixlysia 集成了 [Pino](https://github.com/pinojs/pino) 作为底层日志引擎。Pino 在首次访问时按需构造 —— 错误的 Pino 选项只在真正 log 时才报错。
...
或者通过请求作用域 `log`(`useAsyncLocalStorage: true` 时):
...
**注意:** 在 route 外用 Pino 没有 HTTP context(method、pathname、IP 等)。HTTP 请求日志请在 route handler 里用 `store.logger`。
...
`prettyPrint` 透传给 [pino-pretty](https://github.com/pinojs/pino-pretty)。用的话装一下 `pino-pretty` peer dep。

### 脱敏

Pino 自己的 `redact` 跟 Logixlysia 的 `autoRedact` 独立,可以同时跑:

```ts
logixlysia({
  config: {
    autoRedact: true,                 // Logixlysia: 改写 meta + headers
...
**重要:** Pino transport 和 Logixlysia transport 是独立的:

- **Pino transport**(`pino.transport`)—— 影响 `store.pino.info()`、`store.pino.error()` 等写出的日志
- **Logixlysia transport**(`config.transports`)—— 影响 HTTP access log 管道
...
logixlysia({
  config: {
    ...
    // Logixlysia transport —— 只对 HTTP access log 生效
...
app.get('/example', ({ store }) => {
  ...
  // HTTP access log 走 Logixlysia transport
})
```
...
// ... 插件构造完成,还不会报错
// ... 第一次 plugin.store.pino.info() 调用时 throw
```

想强制启动时立即初始化(让错误的 Pino 选项 fail-fast),把 `config.pino` 设为真值。Pino 实例在插件构建时就创建。
...
4. **脱敏** —— `autoRedact`(Logixlysia)和 `pino.redact`(Pino)都开
