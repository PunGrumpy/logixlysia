...
new Elysia().use(createLogPlugin({ preset: 'prod' }))
```
...
```ts
import { mergeAIMetrics } from 'createLogPlugin/ai'
...
Use `injectTraceContext` from `createLogPlugin/otel` in the `request` hook (see [OpenTelemetry](/docs/integrations/otel)).
