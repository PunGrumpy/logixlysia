...
The `next` tag targets the Elysia 2 beta. The `latest` tag stays on Elysia 1.4. See [Use Logixlysia with Elysia 2](/docs/use-with-elysia-2).

### What are the requirements?

- Node.js 22+ or Bun
- Elysia 2.0.0-beta.19 or newer (`logixlysia@next`), or Elysia 1.4 (`logixlysia@latest`)
- TypeScript 5.7+

### How do I install a preview build for an unreleased fix?

Every commit on `main` and every open pull request publishes an installable preview build to [pkg.pr.new](https://pkg.pr.new). Install by commit SHA or PR number:
...
Reach for a preview build when you want to reproduce a bug against the latest `main`, or verify that a fix in an open PR resolves your issue. These builds bypass the npm registry and are for testing only — do not use them in production. See [Preview Releases](/contributing#preview-releases) for details.
...
logixlysia({
  config: {
    transports: [elasticsearchTransport, slackTransport, mongodbTransport]
  }
})
```
