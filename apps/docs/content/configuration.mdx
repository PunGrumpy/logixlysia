...
完整 Logixlysia 配置选项参考。所有字段都放在 `createLogPlugin({...})` 的 `config` 键下,根级有一个 `logLevel` 简写。
...
`verbose` 开启时,createLogPlugin 会在 level 派生的日志之上,再写一条结构化 warn,带上完整错误上下文。
