---
title: Configuration
description: Logixlysia 完整配置参考
---

完整 Logixlysia 配置选项参考。所有字段都放在 `logixlysia({...})` 的 `config` 键下,根级有一个 `logLevel` 简写。
...
关闭 `app.ws(...)` 路由通过 `plugin.wrapWs(...)` 时的自动日志。
...
`verbose` 开启时,logixlysia 会在 level 派生的日志之上,再写一条结构化 warn,带上完整错误上下文。
