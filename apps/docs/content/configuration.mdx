...
Value-pattern redaction is best-effort — it only catches values matching those specific shapes. Always prefer key-based redaction (`redactKeys`) for known sensitive fields instead of relying on their value happening to match a pattern. For free-text PII that has no fixed shape — names, street addresses, phone numbers, national IDs — see [neural PII redaction](/docs/integrations/desertant), which runs an on-device model over records on their way to a transport.
...
Additional key/header names to redact when `autoRedact` is enabled, extending the built-in denylist. Matching is case-insensitive and normalizes `-`, `_`, and camelCase variants. The same key-name matching also applies to query-string parameter names, since transports receive the full request URL.
...
Timestamp configuration for the `{now}` console token.

- **Type:** `{ translateTime?: string }`
- **Default:** `undefined`

`translateTime` accepts three forms:

- A bare token pattern (`yyyy`, `mm`, `dd`, `HH`, `MM`, `ss`, `SSS`), rendered with local time:
...
- pino-pretty's `SYS:` prefix, also local time — `SYS:standard` expands to `yyyy-mm-dd HH:MM:ss.SSS`, or supply your own pattern after the prefix:
...
- pino-pretty's `UTC:` prefix, rendered with UTC getters instead of local time:
...
Called when a sink (transport, file, or rotation) or an [enricher](/docs/features/enrichers) fails, and when a shutdown flush runs out of time (`sink: 'shutdown'`). Errors thrown by the hook itself are swallowed. When absent, failures go to stderr (rate-limited for transports and enrichers).

- **Type:** `(context: { sink: 'transport' | 'file' | 'rotation' | 'shutdown' | 'enricher'; error: unknown }) => void`
- **Default:** `undefined`
...
How long the plugin waits for transports and the file sink to flush when the app stops. `0` starts the flush but does not wait. A flush that exceeds the budget is reported through `onError` with `sink: 'shutdown'`.
...
Restricts which log levels get processed. See [Filtering](/docs/features/filtering).
