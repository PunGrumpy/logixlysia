...
Colors require an interactive terminal: when `stdout` is not a TTY (Docker, CI, or output piped to a file), Logixlysia prints logs uncolored even with `useColors: true`.
...
Reverse proxies (nginx, Caddy, Cloudflare, etc.) and load balancers typically set these headers. When testing locally (localhost or LAN) without a proxy, these headers are usually absent, so the `{ip}` placeholder will be empty. To test IP logging locally, you can pass a header manually, e.g. `curl -H 'x-real-ip: 1.2.3.4' http://localhost:3000/`.

### `autoRedact`

Automatically redacts sensitive data from log messages, context objects, errors, and request headers before Logixlysia outputs them. Redaction runs in two ways:
...
Called when a sink (transport, file, or rotation) or an [enricher](/docs/features/enrichers) fails. Logixlysia swallows errors thrown by the hook itself. When absent, failures go to stderr (rate-limited for transports and enrichers).
