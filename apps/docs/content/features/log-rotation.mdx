...
> [!NOTE] Interval rotation is evaluated when a log line is written, not on a wall-clock timer. If the process writes nothing for longer than `interval`, the file rotates on the next write — an idle process's file can exceed `interval` in age until traffic resumes. The file's age is read from filesystem creation time, so it survives restarts.
