...
Elogs 的 rotation 在 file sink 内部跑 —— [`FileSink`](/docs/api/types#file-sink) 每次 flush 后检查触发条件,触发就把当前文件改名、压缩、按保留策略清理。整套行为只用一个 [`LogRotationConfig`](/docs/api/types#log-rotation-config) 块配置。
...
完整类型见 [`LogRotationConfig`](/docs/api/types#log-rotation-config)。
...
压缩跑在 keyed mutex 锁内,跨文件互不阻塞;但同文件并发 rotation 会被串行化。压缩失败不会让主流程挂掉 —— 错误经 [`RotationErrorReporter`](/docs/api/types#rotation-error-reporter) 上报,默认打 `console.error`。
...
- [`ElogsConfig.logRotation`](/docs/api/configuration#elogs-config) — rotation 配置入口
- [`LogRotationConfig`](/docs/api/types#log-rotation-config) — 字段类型
- [`FileSink`](/docs/api/types#file-sink) — 内部 file sink 抽象
- [`shouldRotate`](/docs/api/exports#shouldrotate) — 大小触发的预检查
- [`performRotation`](/docs/api/exports#performrotation) — 执行 rename + compress + cleanup
- [`rotateFile`](/docs/api/exports#rotatefile) — 单次 rename
- [`compressFile`](/docs/api/exports#compressfile) — 单次 gzip
- [`getRotatedFileName`](/docs/api/exports#getrotatedfilename) — 生成归档文件名
- [`RotationErrorReporter`](/docs/api/types#rotation-error-reporter) — 错误回调签名
