...
- [`RequestIdConfig`](/docs/api/types#requestidconfig) — 配置类型
- [`ResolvedRequestIdConfig`](/docs/api/types#resolvedrequestidconfig) — `resolveRequestIdConfig` 返回的归一化形态
- [`resolveRequestIdConfig`](/docs/api/exports#resolverequestidconfig) — 归一化 `boolean | object` → `ResolvedRequestIdConfig | null`
- [`getOrCreateRequestId`](/docs/api/exports#getorcreaterequestid) — 读 header 或生成新值
