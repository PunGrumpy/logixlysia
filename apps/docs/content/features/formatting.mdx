...
If a caught error contains any of these fields, Logixlysia automatically extracts them and renders them under the `error.` namespace (`error.why`, `error.fix`, `error.link`, and `error.internal`) as individual tree branches.
