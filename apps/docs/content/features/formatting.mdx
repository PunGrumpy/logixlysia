...
- [`ElogsConfig.customLogFormat`](https://elogs.vercel.app/docs/api/configuration#elogs-config) — 模板字符串
- [`ElogsConfig.timestamp`](https://elogs.vercel.app/docs/api/configuration#elogs-config) — `string | { format: string }`
- [`ElogsConfig.useColors`](https://elogs.vercel.app/docs/api/configuration#elogs-config) / `ip` / `showIp` / `logQueryParams` / `service` / `slowThreshold` / `verySlowThreshold` / `showContextTree` / `contextDepth`
- [`FormatContext`](https://elogs.vercel.app/docs/api/types#format-context) — per-logger 提升的格式常量(token set、template、useColors、thresholds)
