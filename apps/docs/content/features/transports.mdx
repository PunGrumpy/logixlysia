...
`log` is the only required member. Implement `flush` when your transport batches or queues entries — the plugin calls it on [shutdown](/docs/adapters/overview) so the last lines before an exit still arrive. Implement `close` when you hold a socket, timer, or buffer that must be released; it is never called automatically, only through `flushLogixlysia(options, { close: true })`, and it must be idempotent.
