...
- [`createWsHandlerWrapper`](/docs/api/exports#createwshandlerwrapper) — WebSocket 事件 wrap 工厂
- [`WebSocketLike`](/docs/api/types#websocketlike) — 最小 WebSocket 协议
- [`WsHandlerHooks`](/docs/api/types#wshandlerhooks) — `open` / `message` / `close` hook 形状
- [`disableWebSocketLogging`](/docs/api/types#elogsconfig) — `ElogsConfig` 上的开关
- [`ContextKey`](/docs/api/types#contextkey) — WebSocket 实例是合法的 context key
