...
Parses the W3C `traceparent` header directly, with **no OpenTelemetry SDK required**. That is the difference from [`logixlysia/otel`](/integrations/otel), which reads the ids of the currently active span and therefore needs the SDK installed and instrumenting your app.
...
Failures go to [`onError`](/configuration#onerror) with `sink: 'enricher'`, or to stderr (rate-limited) when no hook is configured.
...
Enricher fields live in the request context, which is merged at emit time. A record rescued by [tail sampling](/features/sampling) is replayed after the response phase has run, so replayed records carry the response-phase fields too.
