...
The response phase runs before the request's final log line on both the success and the error path. Its fields are therefore on the access log and on the error log alike. Request-phase fields, being merged at the start, also reach any `log.info()` you make inside the handler.
