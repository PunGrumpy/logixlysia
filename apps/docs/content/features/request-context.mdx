...
`useLogger()` 走的是 Node.js `AsyncLocalStorage`,在 logixlysia 自己的 `.request()` 钩子里用 `enterWith()` 设置 scope。Elysia 2 内部对每个请求会包一层自己的 `als.run()` scope,所以 `enterWith` 设进去的值能**透传**到路由 handler 和 `.afterHandle()`。
