...
用 Elogs 的 file sink 把日志写到本地文件,持久化、归档、离线分析都用得上。所有文件行为都集中在 [`ElogsConfig`](/docs/api/types#elogs-config) 的几个字段上,不需要碰 plugin 选项。
...
| `logRotation` | [`LogRotationConfig`](/docs/api/types#log-rotation-config) | 轮转策略(大小 / 间隔 / 保留 / 压缩) | `undefined` |
| `disableFileLogging` | `boolean` | 即使配了 `logFilePath` 也关掉 file sink | `false` |

字段定义见 [`ElogsConfig`](/docs/api/configuration#elogs-config),类型见 [`LogRotationConfig`](/docs/api/types#log-rotation-config)。

## 工作机制

file sink 走的是单 emit 管道的 file 出口([`emit.ts`](/docs/api/exports#emit)),核心四点:
...
- [`ElogsConfig.logFilePath`](/docs/api/configuration#elogs-config) — 启用文件输出
- [`ElogsConfig.logFileMode`](/docs/api/configuration#elogs-config) — 文件权限
- [`ElogsConfig.logDirMode`](/docs/api/configuration#elogs-config) — 目录权限
- [`ElogsConfig.logRotation`](/docs/api/configuration#elogs-config) — 轮转配置
- [`ElogsConfig.disableFileLogging`](/docs/api/configuration#elogs-config) — 关闭 file sink
- [`LogRotationConfig`](/docs/api/types#log-rotation-config) — rotation 字段类型
- [`FileSink`](/docs/api/types#file-sink) / [`FileSinkOptions`](/docs/api/types#file-sink-options) — 底层 file sink 抽象
- [`getFileSink(path)`](/docs/api/exports#getfilesink) — 按 path 拿复用 sink
- [`logToFile`](/docs/api/exports#logtofile) — 内部 file 出口
