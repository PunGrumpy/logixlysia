...
Tail buffering applies to HTTP requests the plugin opens and closes. WebSocket lifecycle logs are head-sampled but never buffered, because a long-lived socket has no request end to rescue against. The same is true for a `createLogger` instance used outside the plugin.
