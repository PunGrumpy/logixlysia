...
Unlike [log filtering](/features/filtering), which is all-or-nothing per level, sampling is per-record and reversible.
...
Sampling runs before every sink, so a dropped record never reaches [transports](/features/transports), file logging, or the console. That is the point: it is the cheapest place to cut what Datadog, Better Stack, or PostHog will charge you for.
