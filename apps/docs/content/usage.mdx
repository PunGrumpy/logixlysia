...
The `next` tag tracks the [Elysia 2 open beta](/docs/elysia-2). For Elysia 1.4, install `logixlysia` from the default `latest` tag instead.
...
logixlysia({
  config: {
    customLogFormat:
      '{now} {level} {duration}ms {method} {pathname}{query} {status}'
  }
})
```
