...
`next` 标签对应 [Elysia 2 open beta](/docs/elysia-2)。如果你还在 Elysia 1.4,装上游的 `createLogPlugin` 包(`latest` 标签)。
...
`createLogPlugin()` 返回一个插件。`Logger` 挂在 Elysia store 上,可以通过 `store.logger` 访问;请求作用域的 `log` 也会派生到 handler context 上。插件同时注册 `.request`、`.afterHandle`、`.error` 和 `.start` 钩子,你不需要手动接线。
...
```ts
createLogPlugin({ preset: 'dev' })   // 美化 console + 启动 banner
createLogPlugin({ preset: 'prod' })  // JSON console + autoRedact + requestId
createLogPlugin({ preset: 'json' })  // 极简 JSON console,无附加
```
...
```ts
createLogPlugin({ config: { requestId: true } })
```
...
```ts
createLogPlugin({ config: { useAsyncLocalStorage: true } })
