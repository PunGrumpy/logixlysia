...
Elysia 2 removed the `code` string from the error context. Narrow with `instanceof`, or register a handler for one class with `.error(MyError, fn)`:

```ts
app.error(({ error, store }) => {
  const { pino } = store
