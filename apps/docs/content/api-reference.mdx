...
The plugin declares its own `LogixlysiaSingleton` with a closed `store`, a `derive` slot that holds `log`, and an empty `decorator` slot. It avoids `SingletonBase`'s wide `Record<string, unknown>`, which would lose the exact keys. It also avoids `Record<string, never>`, which would intersect Elysia's `Context` keys with `never`. As a result, `Context` and the WebSocket handler context keep exact types after `.use(logixlysia())`.
