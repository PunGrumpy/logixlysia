...
The plugin uses an explicit `LogixlysiaSingleton`: a closed `store`, a `derive` slot holding `log`, and an empty `decorator` slot, which avoid both `SingletonBase`’s wide `Record<string, unknown>` and `Record<string, never>` (the latter would intersect Elysia’s `Context` keys with `never`). That keeps `Context` and the WebSocket handler context precise after `.use(logixlysia())`.
