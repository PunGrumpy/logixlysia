...
The benchmark package has three request-path suites: an **overhead floor** (all sinks disabled, measuring plugin dispatch cost only), a **structured sink** run (no-op transport/drain, measuring log assembly and dispatch), and a **file sink** run. Compare like with like: the floor numbers contain no logging work by design. Results vary by machine, so use them for regression tracking, not absolute rankings.
