# Elysia Node Example with Logixlysia

Example application demonstrating Logixlysia logging plugin with Elysia.js and Node.js adapter.

![Preview](./public/preview.png)

## Overview

This example shows how to integrate Logixlysia into an Elysia.js application using the Node.js adapter, including:
...
The server will start on `http://localhost:3002` (or the port specified in `PORT` environment variable).
...
Visit `http://localhost:3002/swagger` to see the interactive API documentation.
...
```bash
curl http://localhost:3002/status/200
curl http://localhost:3002/status/404
curl http://localhost:3002/status/500
```
...
```bash
curl http://localhost:3002/custom
```
...
## Differences from Elysia Example

This example uses `@elysiajs/node` adapter instead of Bun's native server, making it compatible with Node.js runtime.
