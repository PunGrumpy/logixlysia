...
You can extend Logixlysia by creating custom log handlers:

```ts
import { LogHandler } from 'logixlysia'

class CustomHandler implements LogHandler {
  handle(log: LogEntry) {
...
Check out the [API Reference](/docs/api) for detailed information about all available options and features.